...
class TBall;
void ZeichneFeld(TBall &ball, const double feldX, const double feldY);	//"const" hier entfernt, sonst Fehler!
...
public:
  TBall(const double X, const double Y, const double feldX, const double feldY, const double dt)
  {
    mX = X;
    mY = Y;
    mDt = dt;
  ...
  void Fliege()
  {
    ...
    mY += mVy*mDt;
     ...
     if(mX > feldarrayX || mX < 0)
     {
       deltaX = mX - feldarrayX;
       fabs(mX = feldarrayX - deltaX);       //Rückgabe des Betrags, um beide Fälle abzudecken
     }

     if(mY >= feldarrayY || mY < 0)
     {
       deltaY = mY - feldarrayY;
       mY = feldarrayY - deltaY;
       fabs(mY = feldarrayY - deltaY);       //Rückgabe des Betrags, um beide Fälle abzudecken
     }
  }
...
void ZeichneFeld(TBall &ball, const double feldX, const double feldY)     //"const TBall" hier entfernt, sonst Fehler!
{
